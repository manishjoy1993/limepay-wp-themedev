...
if( class_exists('WP_Customize_Control')):
    ...
    */
    class Ecommerce_Shop_Repeater_Controler extends WP_Customize_Control {
    	/**
    	 * The control type.
    	 *
    	 * @access public
    	...
    	*/
    	...
    	public $ecommcer_shop_box_label = '';

    	public $ecommcer_shop_box_add_control = '';
    	...
    	public function __construct( $manager, $id, $args = array(), $fields = array() ) {
    		$this->fields = $fields;
    		$this->ecommcer_shop_box_label = $args['ecommcer_shop_box_label'] ;
    		$this->ecommcer_shop_box_add_control = $args['ecommcer_shop_box_add_control'];
    	...
    	public function render_content() {
    		...
    		<ul class="ecommerce-shoprepeater-field-control-wrap">
    			<?php
    			$this->ecommcer_shop_get_fields();
    			?>
    		</ul>

    		<input type="hidden" <?php esc_attr( $this->link() ); ?> class="ecommerce-shoprepeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
    		<button type="button" class="button ecommerce-shopadd-control-field"><?php echo esc_html( $this->ecommcer_shop_box_add_control ); ?></button>
    		<?php
    	}

    	private function ecommcer_shop_get_fields(){
    		$fields = $this->fields;
    		$values = json_decode($this->value());
    		...
    		?>
    		<li class="ecommerce-shoprepeater-field-control">
    		<h3 class="ecommerce-shoprepeater-field-title"><?php echo esc_html( $this->ecommcer_shop_box_label ); ?></h3>
    		
    		<div class="ecommerce-shoprepeater-fields">
    		<?php
    			foreach ($fields as $key => $field) {
    			$class = isset($field['class']) ? $field['class'] : '';
    			?>
    			<div class="ecommerce-shopfields ecommerce-shoptype-<?php echo esc_attr($field['type']).' '.esc_attr($class); ?>">
	    			<?php 
	    				...
	    				switch ($field['type']) {
	    					...
	    					case 'upload':
	    						...
	    						echo '<div class="ecommerce-shopfields-wrap">';
	    						echo '<div class="attachment-media-view">';
	    						echo '<div class="placeholder'.esc_attr($image_class).'">';
	    						esc_html_e('No image selected', 'ecommerce-shop');
	    						echo '</div>';
	    						echo '<div class="thumbnail thumbnail-image">';
	    						echo esc_url($image);
	    						echo '</div>';
	    						echo '<div class="actions clearfix">';
	    						echo '<button type="button" class="button ecommerce-shopdelete-button align-left">'.esc_html__('Remove', 'ecommerce-shop').'</button>';
	    						echo '<button type="button" class="button ecommerce-shopupload-button alignright">'.esc_html__('Select Image', 'ecommerce-shop').'</button>';
	    					...
	    					case 'category':
	    						...
	    						echo '<option value="0">'.esc_html__('Select Category', 'ecommerce-shop').'</option>';
	    						echo '<option value="-1">'.esc_html__('Latest Posts', 'ecommerce-shop').'</option>';
	    					...
	    					case 'colorpicker':
	    						echo '<input data-default="'.esc_attr($default).'" class="ecommerce-shopcolor-picker" data-alpha="true" data-name="'.esc_attr($key).'" type="text" value="'.esc_attr($new_value).'"/>';
	    						break;
                            
                            case 'page':
                                echo '<select data-default="'.esc_attr($default).'"  data-name="'.esc_attr($key).'">';
	    						echo '<option value="0">'.esc_html__('Select Page', 'ecommerce-shop').'</option>';
	    					...
	    					case 'range':
	    						...
	    						echo '<div class="ecommerce-shoprange-slider" >';
	    					...
	    					case 'icon':
	    						echo '<div class="ecommerce-shopselected-icon">';
	    						echo '<i class="'.esc_attr($new_value).'"></i>';
	    						echo '<span><i class="fa fa-chevron-down"></i></span>';
	    						echo '</div>';
	    						echo '<ul class="ecommerce-shopicon-list clearfix">';
	    						$ecommcer_shop_icons_array = ecommcer_shop_icons_array();
	    						foreach ($ecommcer_shop_icons_array as $ecommcer_shop_font_awesome_icon) {
	    							$icon_class = $new_value == $ecommcer_shop_font_awesome_icon ? 'icon-active' : '';
	    							echo '<li class='.esc_attr($icon_class).'><i class="fa '.esc_attr($ecommcer_shop_font_awesome_icon).'"></i></li>';
	    						}
	    						echo '</ul>';
	    						echo '<input data-default="'.esc_attr($default).'" type="hidden" value="'.esc_attr($new_value).'" data-name="'.esc_attr($key).'"/>';
	    						break;
	    					...
	    					case 'multicategory':
	    						...
	    						echo '<ul class="ecommerce-shopmulti-category-list">';
	    						echo '<li><label><input type="checkbox" value="-1" '. checked('-1', $new_value, false ) .'/>'.esc_html__( 'Latest Posts', 'ecommerce-shop' ).'</label></li>';
	    						foreach ( $this->cats as $cat ){
    			...
    			<div class="clearfix ecommerce-shoprepeater-footer">
    				<div class="alignright">
    				<a class="ecommerce-shoprepeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'ecommerce-shop') ?></a> |
    				<a class="ecommerce-shoprepeater-field-close" href="#close"><?php esc_html_e('Close', 'ecommerce-shop') ?></a>
    				</div>
    			</div>
    		</div>
    		</li>
