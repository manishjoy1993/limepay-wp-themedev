    ...
    $( document ).ready( function( $ ){
        ...
        $( '.urus_upload_image_button').on( 'click', function( event ){
            ...
            file_frame[id] = wp.media.frames.downloadable_file = wp.media( {
                title: urus_brand.labels.upload_file_frame_title,
                button: {
                    text: urus_brand.labels.upload_file_frame_button
                },
                multiple: false
            } );
        ...
        $( '.urus_remove_image_button').on( 'click', function( event ){
            ...
            t.parent().prev().find( 'img' ).attr( 'src', urus_brand.wc_placeholder_img_src );
            t.hide();
            return false;
        } );
        // hide remove button when not needed
        $( '.urus_upload_image_id' ).each( function(){
            var t = $(this);
