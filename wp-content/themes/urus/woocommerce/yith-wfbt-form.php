<?php
    ...
    foreach( $products as $current_product ) {
        ...
        if( $is_wc_30 ) {
            ...
            $current_product_id = $current_product->is_type( 'variation' ) ? $current_product->variation_id : $current_product->get_id();
        }
        ...
        <li class="yith-wfbt-item">
            <label>
				...
				<?php echo ! $index ? esc_html__( 'This Product', 'urus' ) . ': ' . $current_product_title : $current_product_title; ?>
			</span>
...
<div class="yith-wfbt-section woocommerce">
    <form class="yith-wfbt-form" method="post" action="<?php echo esc_url( $url ) ?>">
        <div class="block-left">
            <?php if( $title != '' ) {
                echo '<h3 class="yith-wfbt-form-title">' . esc_html( $title ) . '</h3>';
            }
            ?>
            <table class="yith-wfbt-images">
                <tbody>
                <tr>
                    <?php echo Urus_Helper::escaped_html($thumbs); ?>
                </tr>
                </tbody>
            </table>
            <ul class="yith-wfbt-items">
                <?php echo Urus_Helper::escaped_html($checks); ?>
            </ul>
        </div>
        <div class="block-right">
            <div class="yith-wfbt-submit-block">
                ...
                <input type="submit" class="yith-wfbt-submit-button button border" value="<?php echo esc_html( $label ); ?>">
            </div>
        </div>
